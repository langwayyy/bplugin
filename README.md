...
- 前向事件按日追加到本地 JSONL 日志并保留最近30天；会话记录行情断档、信号、风控、订单、成交、手续费、滑点、盈亏、回撤和数据在线率，重启后可继续运行。
...
在 IDEA 的 `Settings → Plugins → 齿轮 → Install Plugin from Disk` 中选择 `build/distributions/bplugin-2.3.0.888888-SNAPSHOT.zip`，按 IDE 提示重启。
